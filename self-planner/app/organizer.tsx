...
export default function OrganizerApp() {
  ...
  const [viewerId, setViewerId] = useState<"me" | "partner">("me");
    ...
    <main className="app-shell">
      <aside className="side-nav" aria-label="Основная навигация">
        ...
        <button
          ...
          onClick={() =>
            setViewerId((current) => (current === "me" ? "partner" : "me"))
          }
          title="Переключить демо-пользователя"
          type="button"
        >
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            {(offline || loading) && (
              <span className={`sync-pill ${loading ? "is-loading" : ""}`}>
                {loading ? "Синхронизация" : "Демо-режим"}
              </span>
            )}
            <button
              ...
              aria-label="Переключить участника"
              type="button"
            >
              ...
              <span>{viewerId === "me" ? "Вы вдвоём" : "Партнёр сейчас"}</span>
            </button>
            <button
